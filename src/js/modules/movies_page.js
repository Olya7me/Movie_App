...
const filmsTitle = document.querySelector('.movies-page__films h2.movies-page__films-title');
...
async function getAllMovies() {
    try {
        ...
        currentIndex = 0;
        renderMovies(allMovies, filmsItems, filmsButton, scrollToTopButton, createMovieElement);
...
function createMovieElement(movie) {
    ...
    movieElement.innerHTML = `
        ...
        <div class="movies-page__films-item-info">
            ...
            <p class="movies-page__films-item-genre">${movie.genres.map(genre => `${genre.genre}`).join(', ')}</p>
        </div>
    `;
    return movieElement;
}
...
export function initEventListeners() {
    filmsButton.addEventListener("click", () => renderMovies(allMovies, filmsItems, filmsButton, scrollToTopButton, createMovieElement));
